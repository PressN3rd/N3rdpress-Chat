...
(function ($) {
	jQuery(document).ready(function () {
		...
		jQuery('select#wpmudev_chat_wp_admin').change(function(){
			var option_selected = jQuery(this).val();

			if (option_selected == "enabled") {
				jQuery('tr.wpmudev_chat_wp_admin_display').show();
				
			} else if (option_selected == "disabled") {
				jQuery('tr.wpmudev_chat_wp_admin_display').hide();
			} 
		});

		jQuery('select#wpmudev_chat_wp_toolbar').change(function(){
			var option_selected = jQuery(this).val();

			if (option_selected == "enabled") {
				jQuery('div#wpmudev_chat_wp_toolbar_options').show();
				
			} else if (option_selected == "disabled") {
				jQuery('div#wpmudev_chat_wp_toolbar_options').hide();
			} 
		});

		jQuery('select#wpmudev_chat_dashboard_friends_widget').change(function(){
			var option_selected = jQuery(this).val();

			if (option_selected == "enabled") {
				jQuery('div#wpmudev_chat_dashboard_friends_widget_options').show();
				
			} else if (option_selected == "disabled") {
				jQuery('div#wpmudev_chat_dashboard_friends_widget_options').hide();
			} 
		});

		jQuery('select#wpmudev_chat_dashboard_widget').change(function(){
			var option_selected = jQuery(this).val();

			if (option_selected == "enabled") {
				jQuery('div#wpmudev_chat_dashboard_widget_options').show();
				
			} else if (option_selected == "disabled") {
				jQuery('div#wpmudev_chat_dashboard_widget_options').hide();
			} 
		});
